...
public class XMLOrganizer
{
  ...
  public void flatXML(SQLiteAccess sqliteDB, XMLGenerator xmlOut) throws DBException {
    for (String table : sqliteDB.getTableList()) {
      List<Integer> count = sqliteDB.singleColumnQuery(SQLExpression.fillTemplate("select count(*) as count from $1",
          table), "count", Integer.class);
      if (count.size() <= 0)
        continue;
      ...
      while (readRowCount < rowCount) {
        List<JSONObject> rowData = sqliteDB.jsonQuery(SQLExpression.fillTemplate("select * from $1 limit $2 offset $3",
            table, limit, readRowCount));
  ...
  class RecursiveAggregator
  {
    ...
    public int countQuery(String tableName, List<String> conditionList) throws DBException {
      List<Integer> count = sqliteDB
          .singleColumnQuery(SQLExpression.fillTemplate("select count(*) as count from $1 $2", tableName,
              generateWhereClause(conditionList)), "count", Integer.class);
    ...
    public Count getMinGroupCount(String tableName, List<DataType> remainingDataTypeList, List<String> conditionList)
      ...
      String sql = SQLExpression.fillTemplate("select $1 from (select * from $2 $3)", selectColumnStmt, tableName,
          generateWhereClause(conditionList));
    ...
    public void recursivelyOutputColumnData(String tableName, LinkedList<DataType> columnDataTypeList,
      ...
      if (minCount.count >= rowCount) {
        ...
        List<String> selectColumnList = CollectionUtil.collect(columnDataTypeList, new Functor<DataType, String>() {
        ...
        while (readRowCount < rowCount) {
          List<JSONObject> rowData = sqliteDB.jsonQuery(SQLExpression.fillTemplate(
              "select $1 from $2 $3 limit $4 offset $5", selectColumn, tableName, condition, limit, readRowCount));
          for (JSONObject obj : rowData) {
